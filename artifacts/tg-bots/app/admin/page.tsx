                  ...
                  <ResponsiveContainer width="100%" height={280}>
                    <BarChart data={topBots} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                      ...
                      <Tooltip
                        ...
                        formatter={(value: number, _: string, props: { payload: { botName: string } }) => [
                          `${value} просм.`,
                        ...
                        {(stats ?? []).map((stat, i) => {
                          ...
                          return (
                            <tr key={stat.botId} className="border-b border-white/5 hover:bg-white/[0.03] transition-colors">
                              ...
                              <td className="px-4 py-3 text-right text-sm text-white">{stat.last7Days}</td>
                              <td className="px-4 py-3 text-right text-sm text-white">{stat.last30Days}</td>
                              <td className="px-4 py-3 text-right text-sm font-semibold text-primary">{stat.totalViews}</td>
                            </tr>
                          );
                        })}
                      </tbody>
        ...
        {deleteConfirm && (
          ...
          >
            <motion.div
              ...
              className="glass-panel rounded-2xl border border-white/10 p-6 w-full max-w-sm"
            >
              <h3 className="font-bold text-white text-lg mb-2">Удалить бота?</h3>
              <p className="text-muted-foreground text-sm mb-6">
                Вы уверены, что хотите удалить <span className="text-white font-medium">{deleteConfirm.name}</span>? Это действие нельзя отменить.
              </p>
              <div className="flex gap-3">
                ...
                >
                  {deletingId === deleteConfirm.id ? "Удаление..." : <><Trash2 className="w-4 h-4" /> Удалить</>}
                </button>
              </div>
            </motion.div>
          </motion.div>
